...
<SideNote>
  Generally, error handling based on exceptions is [not the best
  option](https://youtu.be/T6Os27MKUCQ). We could use functional techniques for error handling, but
  we'll talk about them separately someday. I hope I'll live long enough to write about it in detail
  with examples.
</SideNote>
