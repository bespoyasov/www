...
![Converter with a base currency input field, a quote currency selector, and a quotes update button](/images/content/explicit-design-series/03-converter.png)
...
![Diagram of components and the data they display to the user](/images/content/explicit-design-series/03-state-model.png)
...
In code, we will strive to emphasize these dependencies and separate different types of state. We won't always physically separate them (in the file system), but at least we'll try to do it conceptually—in the types and values of variables passed to components.
...
![The application core “communicates” with the user interface through “handles” and “slots” of user input and output](/images/content/explicit-design-series/03-handles-and-slots.png)
...
![Components translate user intentions between the application core and the UI](/images/content/explicit-design-series/03-ui-border.png)
...
![Changes to UI components don't affect the core, because all communication happens through immutable ports. This allows us to use different components for the same use case or piece of data in the model](/images/content/explicit-design-series/03-ui-variants.png)
...
Note that in the test we are only checking the code that is _before the port_. Everything that happens after calling `updateBaseValue` is already [_not the responsibility of the component_](https://solidbook.vercel.app/srp). Its job is to call a certain function with the right parameters, and then the application takes responsibility for executing it.
...
![Presentational components are “separated” from the application by containers. They don't know where their data comes from or where events go—that's the job of “containers”](/images/content/explicit-design-series/03-presentational-components-and-containers.png)
...
![The `BaseValueInput` container “translates” presentational component events into application signals and provides it with all necessary data](/images/content/explicit-design-series/03-base-value-input.png)
...
<SideNote>
  ...
  status](https://github.com/bespoyasov/refactor-like-a-superhero/blob/main/manuscript-ru/09-functional-pipeline.md#невыразимость-неправильного).
</SideNote>
